...
def load_smileys(path: str) -> dict[str, list[str]]:
    """
    Loads smileys from emoticons.xml file.
...
def unblock_smiley_maybe(
    ...
    """
    Unblocks smiley defined in unblock and return True if blocked smiley list was modified.
...
def load_and_update_blocklist(
    ...
    """
    Loads the smiley block list, update and save it.
    ...
    blocked_smileys: dict[str, dict[str, set[str]]] = {}
    if os.path.isfile(block_list_file):
        with open(block_list_file, "r") as f:
            blocked_smileys: dict[str, set[str]] = json.load(f)
...
def remove_blocked_smileys(
    ...
    """
    Removed all blocked_smileys from smileys.
